...
public class SoftAssert {
  public static void equals(
      List<Location> actual, List<Location> expected, String message, double threshold) {
    if (actual.size() != expected.size()) {
      System.err.println(
          "Actual size :" + actual.size() + " is different than expected size: " + expected.size());
      failure(message);
    }
    ...
    for (int currentIndex = 0; currentIndex < actual.size(); currentIndex++) {
      ...
      if (delta(
          actual.get(currentIndex).getLatitude(),
          expected.get(currentIndex).getLatitude(),
          threshold)) {
        if (delta(
            actual.get(currentIndex).getLongitude(),
            expected.get(currentIndex).getLongitude(),
            threshold)) {
